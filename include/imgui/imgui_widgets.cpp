...
bool ImGui::NavButton(const char* label, bool selected , const ImVec2& size_arg)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(160, 40), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    ImGui::RenderFrame(bb.Min, bb.Max, bg_col, true, style.FrameRounding);

    if (selected) {
        // 如果被选中，在左侧绘制一个高亮条
        window->DrawList->AddRectFilled(
            ImVec2(bb.Min.x, bb.Min.y + 4),
            ImVec2(bb.Min.x + 4, bb.Max.y - 4),
