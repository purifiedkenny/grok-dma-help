...
public:
	...
	* @return true if successful, false if not.
	*/
	bool ReadCacheEx(uintptr_t address, void* buffer, size_t size) const;
	...
	*/
	...
	T ReadCache(uint64_t address)
	{
		T buffer{ };
		memset(&buffer, 0, sizeof(T));
		ReadCacheEx(address, reinterpret_cast<void*>(&buffer), sizeof(T));
	...
	*/
	...
	bool IsValidPointer(const void* ptr, size_t size = sizeof(void*)) {
		if (!ptr) return false;
		...
		if (!(mbi.Protect & PAGE_READONLY) &&
			...
			!(mbi.Protect & PAGE_EXECUTE_READWRITE)) return false;

		// 确保整个对象的内存区域都是可访问的
		if (reinterpret_cast<uintptr_t>(ptr) + size >
			reinterpret_cast<uintptr_t>(mbi.BaseAddress) + mbi.RegionSize) return false;

		return true;
	}

	bool IsValidPointer(const uint64_t address)
	{
		// 空指针和保留内存区域检查
		...
		// 特殊调试模式下的内存模式检查
		if (address == 0xCCCCCCCCCCCCCCCC ||  // 未初始化内存（MSVC）
			address == 0xCDCDCDCDCDCDCDCD ||  // 未初始化堆内存
			address == 0xDDDDDDDDDDDDDDDD ||  // 已释放内存
			address == 0xFDFDFDFDFDFDFDFD ||  // 不可访问边界
			address == 0xFEEEFEEEFEEEFEEE ||  // 堆释放后标记
			address == 0xABABABABABABABAB ||  // 另一种调试标记
			address == 0xBAADF00DBAADF00D ||  // 未初始化堆分配内存
			address == 0xDEADC0DEDEADC0DE)    // 调试标记
			return false;

		// 检查地址是否在特定高地址范围内
		if (address > 0x7FFFFFFFFFFF && !(address >= 0x7FF000000000))
			return false;
	 ...
	 * \param pid
	 */
	void ExecuteReadScatter(VMMDLL_SCATTER_HANDLE handle, int pid = 0,bool print = true);
	bool ExecuteReadScatterBOOL(VMMDLL_SCATTER_HANDLE handle, int pid = 0);
	...
	bool cleanMmapFile(const std::wstring& tempPath) {
		try {
			if (!fs::exists(tempPath)) {
				return false;
			}

			std::wstring mmapPath = tempPath + L"\\mmap.txt";
			if (fs::exists(mmapPath)) {
				fs::remove(mmapPath);
				return !fs::exists(mmapPath);
			}
			return true;
		}
	...
	void CleanMmap()
	{
		...
		if (userProfile != nullptr) {
			std::wstring tempPath = std::wstring(userProfile) + L"\\AppData\\Local\\Temp";
			free(userProfile);

			cleanMmapFile(tempPath);
		}
	}
